    ...
    public function create()
    {
        ...
        foreach ($stepDocuments as $stepDocument) {
            if ($stepDocument->task->id == 1) {
                ...
                $data = ['documents_id' => intval($documents_id), 'states_id' => 3, 'stepsdocuments_id' => intval($stepDocument->id), 'users_id' => intval(\Auth::User()->id)];
            } elseif ($stepDocument->order == $i) {
                $flag = true;
                $data = ['documents_id' => intval($documents_id), 'states_id' => 2, 'stepsdocuments_id' => intval($stepDocument->id), 'users_id' => null];
            } elseif ($stepDocument->order != $i and $flag == false) {
                $flag = true;
                $i = $stepDocument->order;
                $data = ['documents_id' => intval($documents_id), 'states_id' => 2, 'stepsdocuments_id' => intval($stepDocument->id), 'users_id' => null];
            } else {
                $data = ['documents_id' => intval($documents_id), 'states_id' => 1, 'stepsdocuments_id' => intval($stepDocument->id), 'users_id' => null];
            }
