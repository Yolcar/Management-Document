...
class workflowController extends \BaseController {
	...
	public function create()
	{
		...
		foreach ($stepDocuments as $stepDocument)
        {
            if ($stepDocument->task->id == 1){
                ...
                $data = array('documents_id' => intval($documents_id),'states_id' => 3,'stepsdocuments_id' => intval($stepDocument->id),'users_id' => intval(\Auth::User()->id));
            }
			elseif($stepDocument->order == $i){
				$flag = true;
				$data = array('documents_id' => intval($documents_id),'states_id' => 2,'stepsdocuments_id' => intval($stepDocument->id),'users_id' => null);
			}
			elseif($stepDocument->order != $i and $flag == false){
				...
				$data = array('documents_id' => intval($documents_id),'states_id' => 1,'stepsdocuments_id' => intval($stepDocument->id),'users_id' => null);
			}
        ...
        return Redirect::route('workflow.show',$documents_id);
    ...
    public function editDocument($idDocument,$idWorkflow){
        $user = \Auth::User();
        $workflow = $this->workflowRepo->find($idWorkflow);

        if ($workflow){
            if($workflow->documents_id==$idDocument and $workflow->stepdocument->edit == 1){
                $document = $this->documentRepo->find($idDocument);
                return View::make('workflow.editDocument')->with('document',$document)->with('workflow',$workflow);
            } else {
                return Response::view('errors.missing', array(), 404);
            }
        } else {
    ...
    public function saveDocument($idDocument,$idWorkflow){
        ...
        $manager->save();
        return Redirect::route('workflow.action',[$idDocument,$idWorkflow]);
    }
