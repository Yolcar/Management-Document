        ...
        foreach ($tasks_id as $key => $n) {
            if ($groups_id[$key] != null and $order[$key] != null) {
                if (array_key_exists($n, $edit)) {
                    if ($edit[$n] == 1) {
                        $arrData = ['templates_id' => $template_id, 'tasks_id' => $tasks_id[$key], 'groups_id' => $groups_id[$key], 'order' => $order[$key], 'available' => 1, 'edit' => 1];
                    }
                } else {
                    $arrData = ['templates_id' => $template_id, 'tasks_id' => $tasks_id[$key], 'groups_id' => $groups_id[$key], 'order' => $order[$key], 'available' => 1, 'edit' => 0];
                }
